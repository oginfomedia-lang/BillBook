...
export function ImportServicesPage() {
    ...
    const importServices = useBulkImportItems();
            ...
            <div className="grid gap-6 md:grid-cols-3">
                ...
                <div className="md:col-span-2 space-y-4">
                    <form onSubmit={handleSubmit} className="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm space-y-4">
                        ...
                        <div className="flex justify-end gap-3 pt-2">
                            ...
                            <button
                                type="submit"
                                disabled={!file || importServices.isPending}
                            ...
                            >
                                {importServices.isPending ? 'Importing...' : 'Upload & Import'}
                            </button>
                        </div>
                    </form>
                </div>
                ...
                <div className="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm space-y-4 h-fit">
                    ...
                    <div className="rounded-lg bg-slate-50 p-3 font-mono text-[10px] text-slate-600 space-y-2 overflow-x-auto">
                        ...
                        <div>sac, hsn, description, price_expenses, category</div>
                    </div>
                    <div className="space-y-2 text-xs text-slate-600">
                        <div className="flex gap-2">
                            <CheckCircle2 size={14} className="text-emerald-500 flex-shrink-0 mt-0.5" />
                            <span><strong>type</strong>: must be <code className="bg-slate-100 px-1 rounded">service</code>.</span>
                        </div>
                        <div className="flex gap-2">
                            <CheckCircle2 size={14} className="text-emerald-500 flex-shrink-0 mt-0.5" />
                            <span><strong>category</strong>: Must match name of existing category, or will import without category.</span>
                        </div>
                    </div>
                </div>
            </div>
